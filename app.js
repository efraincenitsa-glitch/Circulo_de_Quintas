/* ==========================================================
   Círculo de Quintas – Algorítmico con renderer SVG (Diseño 1 + Glow)
...
function renderCircle(){
  ...
  const outer = 130, inner = 90;
  const step = (2*Math.PI) / notas.length;

  // Defs: gradiente y filtro glow
  const defs = document.createElementNS('http://www.w3.org/2000/svg','defs');
  const rg = document.createElementNS('http://www.w3.org/2000/svg','radialGradient');
  rg.setAttribute('id','gradCore'); rg.setAttribute('cx','50%'); rg.setAttribute('cy','50%'); rg.setAttribute('r','50%');
  ...
  filter.setAttribute('id','glowActive'); filter.setAttribute('x','-50%'); filter.setAttribute('y','-50%'); filter.setAttribute('width','200%'); filter.setAttribute('height','200%');
  const feDrop1 = document.createElementNS('http://www.w3.org/2000/svg','feDropShadow');
  feDrop1.setAttribute('dx','0'); feDrop1.setAttribute('dy','0'); feDrop1.setAttribute('stdDeviation','2.2'); feDrop1.setAttribute('flood-color','#7dd3fc'); feDrop1.setAttribute('flood-opacity','0.9');
  const feDrop2 = document.createElementNS('http://www.w3.org/2000/svg','feDropShadow');
  feDrop2.setAttribute('dx','0'); feDrop2.setAttribute('dy','0'); feDrop2.setAttribute('stdDeviation','5.0'); feDrop2.setAttribute('flood-color','#38bdf8'); feDrop2.setAttribute('flood-opacity','0.55');
  filter.appendChild(feDrop1); filter.appendChild(feDrop2);

  defs.appendChild(rg); defs.appendChild(filter); svg.appendChild(defs);
  ...
  notas.forEach((nota, i)=>{
    ...
    if (nota === sel) {
      ...
      path.setAttribute('filter','url(#glowActive)');
    }
